({
    doCheckContact: function (component, event, helper, connect) {
        ...
        if (
            component.get('v.delegateEmailWasChanged') ||
            (component.get('v.useThisDelegate') && connect)
        ) {
            ...
            if (isValid) {
                ...
                communityService.executeAction(
                    ...
                    function (returnValue) {
                        ...
                        if (returnValue.isDuplicate)
                            delegateItem.isDuplicate = returnValue.isDuplicate;
                        ...
                        if (connect && !returnValue.isDuplicateDelegate)
                            helper.doConnectDelegate(component, event, helper);
    ...
    doConnectDelegate: function (component, event, helper) {
        ...
        communityService.executeAction(
            ...
            function (returnValue) {
                ...
                component.set(
                    'v.delegateItem.continueDelegateMsg',
                    $A
                        .get('$Label.c.PG_Ref_L_Delegate_continue_be_delegate')
                        .replace(
                            '##delegateName',
                            component.get('v.delegateItem.First_Name__c') +
                                ' ' +
                                component.get('v.delegateItem.Last_Name__c')
                        )
                );
                component.find('spinner').hide();
            },
            function (returnValue) {
                ...
                communityService.showErrorToast(
                    '',
                    'Emancipation process - connect/disconnect delegate action failed! Description: ' +
                        returnValue
                );
            }
        );
