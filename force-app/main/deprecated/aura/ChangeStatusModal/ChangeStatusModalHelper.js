({
    ...
    showMessage: function (HCPEnrollment, actionType) {
        ...
        switch (actionType) {
            ...
            case 'orientation-attend-activate':
                message +=
                    $A.get('$Label.c.TST_attended_orientation_and_activated') + ' ' + drName + '.';
                break;
            case 'on-hold':
    ...
    getDoctorName: function (HCPEnrollment) {
        var drName = '',
            drFirstName = '',
            drLastName = '';
        if (HCPEnrollment.HCP_Contact__r && HCPEnrollment.HCP_Contact__r.Salutation)
            drName = HCPEnrollment.HCP_Contact__r.Salutation;
        if (HCPEnrollment.HCP_Contact__r && HCPEnrollment.HCP_Contact__r.FirstName)
            drFirstName = HCPEnrollment.HCP_Contact__r.FirstName;
        if (HCPEnrollment.HCP_Contact__r && HCPEnrollment.HCP_Contact__r.LastName)
            drLastName = HCPEnrollment.HCP_Contact__r.LastName;
    ...
    canChangeStatus: function (enrollment) {
        if (enrollment.Status__c.indexOf('(Admin)') !== -1) {
            communityService.showErrorToast(
                'Error',
                $A.get(
                    '$Label.c.TST_The_status_of_this_record_is_locked_by_site_administration_For_more_inform'
                )
            );
            return false;
        }
