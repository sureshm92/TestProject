...
({
        ...
        communityService.executeAction(
            ...
            function (returnValue) {
                ...
                communityService.showErrorToast(
                    '',
                    'Emancipation process failed! Description: ' + returnValue
                );
            }
        );
    },
    ...
    checkDelegateFields: function (component, event, helper) {
        ...
        for (let ind = 0; ind < delegateItems.length; ind++) {
            helper.checkValidEmail(
                component.find('emailDInput' + ind),
                delegateItems[ind].Email__c
            );
            if (
                delegateItems[ind] &&
                delegateItems[ind].fromStart &&
                (delegateItems[ind].Id ||
                    (delegateItems[ind].First_Name__c &&
                        delegateItems[ind].First_Name__c.trim() &&
                        delegateItems[ind].Last_Name__c &&
                        delegateItems[ind].Last_Name__c.trim()))
            ) {
                isDelegatesValid =
                    isDelegatesValid &&
                    delegateItems[ind].Email__c &&
                    delegateItems[ind].Email__c.trim() &&
                    delegateItems[ind].Phone_Type__c &&
                    delegateItems[ind].Phone__c &&
                    delegateItems[ind].Phone__c.trim() &&
                    (!component.find('emailDInput' + ind) ||
                        (component.find('emailDInput' + ind) &&
                            component.find('emailDInput' + ind).get('v.validity').valid)) &&
                    (!component.find('phoneDInput' + ind) ||
                        (component.find('phoneDInput' + ind) &&
    ...
    initDelegates: function (component, event, helper) {
        communityService.executeAction(
            ...
            function (delegateItems) {
                ...
                for (let ind = 0; ind < delegateItems.length; ind++) {
                    delegateItems[ind].continueDelegateMsg = $A
                        .get('$Label.c.PG_Ref_L_Delegate_continue_be_delegate')
                        .replace(
                            '##delegateName',
            ...
            function (returnValue) {
                ...
                communityService.showErrorToast(
                    '',
                    "Error get Participant's Delegates! Description: " + returnValue
                );
            }
        );
    }
