({
    doInit: function (component, event, helper) {
        ...
        if (wrapper) {
            if (
                (visitResultType === 'Vitals' &&
                    (wrapper.name === 'Weight' || wrapper.name === 'Height')) ||
                (visitResultType !== 'Biomarkers' &&
                    wrapper.minValue == null &&
                    wrapper.maxValue == null) ||
                ...
                if (visitResultType === 'Biomarkers') {
                    if (!wrapper.name.includes('ICOS')) {
                        if (wrapper.value === 1)
                            displayedValue = $A.get('$Label.c.Biomarkers_Positive');
                        else if (wrapper.value === -1)
                            displayedValue = $A.get('$Label.c.Biomarkers_Negative');
                        else if (wrapper.value === 0)
                            displayedValue = $A.get('$Label.c.Biomarkers_Unknown');
