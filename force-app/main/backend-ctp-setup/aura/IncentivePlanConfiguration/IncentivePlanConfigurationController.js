...
({
    ...
    doAddIP: function (component, event, helper) {
        component.find('actionIP').execute(
            null,
            function (ipId) {
    ...
    doCheckboxChange: function (component, event, helper) {
        ...
        component
            ...
            .show(
                $A
                    .get('$Label.c.PG_Ref_L_One_Incentive_Many_Incentives')
                    .replace('{0}', '' + component.get('v.ssItems').length),
                true
            );
    },
        ...
        for (const ssItem of ssItems) {
            for (const assignment of ssItem.assignments) {
                for (const incenitvePlan in allSelectedIPs) {
                    if (
                        assignment.value != ipId &&
                        assignment.value == incenitvePlan &&
                        assignment.state
                    )
                        haveSelecteAll = true;
                }
        ...
        communityService.executeAction(
            ...
            function (returnvalue) {
                ...
                component
                    ...
                    .show(
                        $A
                            .get('$Label.c.PG_Ref_L_Text_To_Delete_Plan')
                            .replace('{0}', '' + returnvalue),
                        false
                    );
            }
        );
