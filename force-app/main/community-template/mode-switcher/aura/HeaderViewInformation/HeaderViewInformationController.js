...
({
    doInit: function (component) {
        ...
        if (currentMode.participantState === 'ALUMNI' && !currentMode.isDelegate)
            label = $A.get('$Label.c.Viewing_as_Self');
        else {
            if (currentMode.userMode === 'Participant')
                label = $A.get('$Label.c.Viewing_as_Participant');
            if (currentMode.userMode === 'PI')
                label = $A.get('$Label.c.Viewing_as_Investigative_Site');
            if (currentMode.userMode === 'HCP')
                label = $A.get('$Label.c.Viewing_as_Referring_Provider');
