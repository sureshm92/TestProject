({
    setData: function (component, pEnroll) {
        ...
        if (RRStatus === 'Pass') {
            ...
            component.set(
                'v.recordReviewMessage',
                $A.get('$Label.c.PG_MRRLI_MSG_Passed_medical_record_review')
            );
        } else if (RRStatus === 'Fail') {
            component.set('v.isReviewPass', false);
            component.set(
                'v.recordReviewMessage',
                $A.get('$Label.c.PG_MRRLI_MSG_Failed_medical_record_review')
            );
        }
        ...
        if (PStatus === 'Excluded from Referring') {
            ...
            component.set(
                'v.referringMessage',
                $A.get('$Label.c.PG_MRRLI_MSG_Excluded_from_referring')
            );
        } else if (PStatus === 'Failed Referral') {
            component.set('v.isReferred', false);
            this.returnPickListTranslation(
                component,
                pEnroll.Non_Referral_Reason__c,
            ...
            if (PreSStatus === 'Pass') {
                component.set(
                    'v.referringMessage',
                    $A.get('$Label.c.PG_MRRLI_MSG_Passed_pre_eligibility_screening_referred')
                );
            } else {
    ...
    returnPickListTranslation: function (component, picklistValue, objectName, fieldName) {
        ...
        action.setCallback(this, function (response) {
            ...
            if (state === 'SUCCESS') {
                ...
                component.set(
                    'v.referringMessage',
                    response.getReturnValue() + ' ' + $A.get('$Label.c.PG_MRRLI_MSG_not_referred')
                );
            } else if (state === 'ERROR') {
