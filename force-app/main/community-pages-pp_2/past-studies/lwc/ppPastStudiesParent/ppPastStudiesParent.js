...
export default class PpPastStudiesParent extends LightningElement {
    ...
    updateSelectedStudy(perid) {
        ...
        for (let i = 0; i < this.perList.length; i++) {
            ...
            if (perid == this.perList[i].Id) {
                ...
                if (
                    this.selectedPER.Clinical_Trial_Profile__r.Visit_Data_Shareback__c &&
                    !this.hideFilesForPER.includes(this.selectedPER.Id)
                )
    ...
    get showFiles() {
        return (
            (!this.showMenu || this.selectedSection == PP_MyFiles) &&
            !this.hideFilesForPER.includes(this.selectedPER.Id)
        );
    }
    get showResults() {
        return (
            (!this.showMenu || this.selectedSection == Visit_Results_Dashboard_My_Results) &&
            !this.hideFilesForPER.includes(this.selectedPER.Id)
        );
    }
    renderSections() {
    ...
    get docFileClass() {
        if (
            ...
            this.docContClass = '';
            this.bottonRightBoxClass =
                'slds-large-size--2-of-5 slds-size--1-of-1 slds-grid slds-wrap';
    ...
    showdeletetoast() {
        this.template
            ...
            .showToast('success', this.label.PP_DeletedSucesfully, 'utility:success', 100000);
    }
}