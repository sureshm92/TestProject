...
({
    checkValidity: function (component, event, helper, stepWrapper) {
        ...
        stepWrapper.formFieldGroups.forEach(function (group) {
            group.fields.forEach(function (field) {
                ...
                if (
                    isCurrentStepValid &&
                    ((field.required &&
                        ((!field.dependent && currentOutcomeSuccess) ||
                            (field.dependent && field.dependentActive)) &&
                        (!field.value ||
                            field.value.trim() === '' ||
                            (field.value == 'false' && field.type == 'checkbox'))) ||
                        field.valid === false)
                ) {
        ...
        isCurrentStepValid =
            isCurrentStepValid &&
            (stepWrapper.outcomeList === undefined ||
                stepWrapper.outcomeList.length == 0 ||
                stepWrapper.outcome === undefined ||
                stepWrapper.outcome === '' ||
                !notesRequired ||
            ...
            for (let i = 0; i < stepWrapper.formFieldGroups.length; i++) {
                for (let j = 0; j < stepWrapper.formFieldGroups[i].fields.length; j++) {
                    for (let k = 0; k < dependentFields.length; k++) {
                        if (
                            dependentFields[k].fieldName ===
                            stepWrapper.formFieldGroups[i].fields[j].field
                        ) {
                            if (
                                dependentFields[k].controllingValue.indexOf(value) !== -1 &&
                                value !== ''
                            ) {
                                stepWrapper.formFieldGroups[i].fields[j].dependentActive = true;
                                if (
                                    stepWrapper.formFieldGroups[i].fields[j]
                                        .populateFromDependent !== null &&
                                    (stepWrapper.formFieldGroups[i].fields[j].value === null ||
                                        stepWrapper.formFieldGroups[i].fields[j].value === '')
                                ) {
                                    for (let k = 0; k < stepWrapper.formFieldGroups.length; k++) {
                                        for (
                                            let l = 0;
                                            l < stepWrapper.formFieldGroups[k].fields.length;
                                            l++
                                        ) {
                                            if (
                                                stepWrapper.formFieldGroups[i].fields[j]
                                                    .populateFromDependent ===
                                                stepWrapper.formFieldGroups[k].fields[l].field
                                            )
                                                stepWrapper.formFieldGroups[i].fields[j].value =
                                                    stepWrapper.formFieldGroups[k].fields[l].value;
                                        }
                                    }
                                }
                            } else {
        ...
        for (let i = 0; i < stepWrapper.formFieldGroups.length; i++) {
            for (let j = 0; j < stepWrapper.formFieldGroups[i].fields.length; j++) {
                if (fieldMap.hasOwnProperty(stepWrapper.formFieldGroups[i].fields[j].field)) {
                    stepWrapper.formFieldGroups[i].fields[j].value =
                        fieldMap[stepWrapper.formFieldGroups[i].fields[j].field];
                }
            }
        }
    }
