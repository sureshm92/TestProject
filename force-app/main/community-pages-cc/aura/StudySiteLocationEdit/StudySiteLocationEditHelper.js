        ...
        communityService.executeAction(
            ...
            function (tmpAccount) {
                if (!tmpAccount.BillingGeocodeAccuracy) {
                    ...
                    acc.BillingStateCode = tmpAccount.BillingStateCode
                        ? tmpAccount.BillingStateCode
                        : null;
                    acc.BillingPostalCode = tmpAccount.BillingPostalCode
                        ? tmpAccount.BillingPostalCode
                        : null;
                    if (
                        tmpAccount.BillingGeocodeAccuracy == 'Address' ||
                        tmpAccount.BillingGeocodeAccuracy == 'NearAddress' ||
                        tmpAccount.BillingGeocodeAccuracy == 'Block' ||
                        tmpAccount.BillingGeocodeAccuracy == 'Street' ||
                        tmpAccount.BillingGeocodeAccuracy == 'ExtendedZip'
                    ) {
                        ...
                        initAcc.BillingStateCode = tmpAccount.BillingStateCode
                            ? tmpAccount.BillingStateCode
                            : null;
                        initAcc.BillingPostalCode = tmpAccount.BillingPostalCode
                            ? tmpAccount.BillingPostalCode
                            : null;
    ...
    checkAccountModified: function (component) {
        ...
        for (let i = 0; i < fieldsToCheck.length; i++) {
            if (currentAccount[fieldsToCheck[i]] != accountInitial[fieldsToCheck[i]]) {
                if (
                    !(currentAccount[fieldsToCheck[i]] == '' && !accountInitial[fieldsToCheck[i]])
                ) {
                    infoWasChanged = true;
                }
            }
            if (
                fieldsToCheck[i].includes('Billing') &&
                currentAccount[fieldsToCheck[i]] != accountInitial[fieldsToCheck[i]] &&
                !addressWasChanged
            ) {
                if (
