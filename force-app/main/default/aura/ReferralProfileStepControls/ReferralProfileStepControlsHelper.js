...
({
    saveSelectedStatus: function (component) {
        ...
        rootComponent.find('changePEStatusByPIAction').execute(
            pe,
            status,
            reason,
            notes,
    ...
    getEmptyFieldNames: function (component, form) {
        ...
        if (!form.get('v.participant').Phone__c)
            fieldNames += ' "Primary daytime telephone number"';
        if (!form.get('v.participant').Phone_Type__c) fieldNames += ' "Phone Type"';
        if (!form.get('v.participant').Email__c && form.find('emailInput').get('v.validity').valid)
            fieldNames += ' "Email address"';
        if (
            !form.get('v.participant').Mailing_Zip_Postal_Code__c ||
            form.get('v.participant').Mailing_Zip_Postal_Code__c === ''
        )
            fieldNames += ' "Zip/Postal code"';

        if (form.get('v.visitPlanRequired') && !form.get('v.pe').Visit_Plan__c)
            fieldNames += ' "Arm/Cohort"';
        if (
            form.find('screeningId').get('v.validity') &&
            !form.find('screeningId').get('v.validity').valid
        )
            fieldNames += ' "Screening/Subject ID"';
        if (form.find('mrnId').get('v.validity') && !form.find('mrnId').get('v.validity').valid)
            fieldNames += ' "MRN/Internal ID"';
