({
    doInit: function (component, event, helper) {
        if (communityService) {
                ...
                for (const item in humanApiVendors) {
                    if (humanApiVendors != null) {
                        isHumanApiVendorChecked =
                            humanApiVendors[item].Medical_Vendor__c === 'HumanApi';
                        break;
                    }
                }
            }
            ...
            if (
                (component.get('v.hasPastStudies') || obj.value == 'ALUMNI') &&
                (!obj.hasPatientDelegates ||
                ...
                communityService.executeAction(
                    ...
                    function (returnValue) {
                        ...
                        if (returnValue) {
                            if (
                                obj.pe != null &&
                                obj.pe.Human_Id__c != null &&
                                obj.pe.Clinical_Trial_Profile__r.Medical_Vendor_is_Available__c &&
                                !component.get('v.isHumanApiChecked')
                            ) {
                                check = true;
                            }
                            ...
                            for (const item in returnValue) {
                                if (
                                    communityService.getCurrentCommunityMode().currentPE &&
                                    obj.pe.Clinical_Trial_Profile__r.Medical_Vendor_is_Available__c
                                ) {
                                    if (
                                        returnValue[item].value.includes(
                                            communityService.getCurrentCommunityMode().currentPE
                                        )
                                    ) {
                            ...
                            if (component.get('v.hasPastStudies')) {
                                if (
                                    component.get('v.referrals').length > 1 &&
                                    component
                                        .get('v.defaultStudy')
                                        .includes(
                                            communityService.getCurrentCommunityMode().currentPE
                                    ...
                                    component.set(
                                        'v.showAuthorizationLink',
                                        component.get('v.isHumanApiChecked')
                                    );
                                } else {
                                    component.set(
                                        'v.showAuthorizationLink',
                                        component.get('v.hasPastStudies')
                                    );
                                }
                            }
            ...
            communityService.executeAction(component, 'getCommunityName', {}, function (
                returnValue
            ) {
                if (returnValue !== null) {
                    var disclaimerText =
    ...
    doListProviders: function (component, event, helper) {
        ...
        if (component.get('v.hasPastStudies')) {
            if (
                component
                    .get('v.defaultStudy')
                    .includes(communityService.getCurrentCommunityMode().currentPE)
            ) {
    ...
    downloadPDF: function (component, event, helper) {
        communityService.executeAction(component, 'downloadBlob', {}, function (returnValue) {
